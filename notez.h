...
#define NOTEZ_DB_INIT "CREATE TABLE IF NOT EXISTS notez" \
    " (id integer primary key, title text, note text, changed datetime)"
#define NOTEZ_DB_LIST_QUERY "SELECT id, strftime(\"%s\", changed)" \
    " AS changed, title FROM notez ORDER BY id DESC"
